...
public class DisplayCutoutEmulationAdapter {
    ...
    private ContentObserver mObserver = new ContentObserver(Handler.getMain()) {
        public void onChange(boolean z) {
            DisplayCutoutEmulationAdapter.update();
        }
    };
    ...
    private void register() {
        mContext.getContentResolver().registerContentObserver(Settings.Global.getUriFor(
                "com.google.android.systemui.display_cutout_emulation"),
                false, mObserver, 0);
    }

    /* access modifiers changed from: private */
    public void update() {
        String stringForUser = Settings.Global.getStringForUser(
                mContext.getContentResolver(), "com.google.android.systemui.display_cutout_emulation", 0);
    ...
    private void setEmulationOverlay(String str) {
        ...
        if ((!TextUtils.isEmpty(str) || !TextUtils.isEmpty(str2)) && !TextUtils.equals(str, str2)) {
            for (OverlayInfo overlayInfo2 : overlayInfos) {
                ...
                if (isEnabled != equals) {
                    try {
                        mOverlayManager.setEnabled(overlayInfo2.packageName, equals, 0);
    ...
    private OverlayInfo[] getOverlayInfos() {
        try {
            List overlayInfosForTarget = mOverlayManager.getOverlayInfosForTarget("android", 0);
